...
if (!testDatabaseUrl) {
  throw new Error(
    'TEST_DATABASE_URL is required for ordinary-TOTP HTTP integration tests.',
  );
}

if (!testMigrationUrl) {
  throw new Error(
    'TEST_DATABASE_MIGRATION_URL is required for ordinary-TOTP HTTP integration tests.',
  );
}
...
test(
  'successful ordinary TOTP MFA returns only admin identity sets the session credential clears the challenge credential advances the replay counter and creates no recovery codes',
  async () => {
    const fixtureNow =
      new Date();
    ...
    const admin =
      await insertAdmin({
        email:
          'http-totp-success@example.test',
...
test(
  'invalid ordinary TOTP returns 401 retains the challenge credential advances the challenge failure count and creates no privileged state',
  async () => {
    const fixtureNow =
      new Date();
    ...
    const admin =
      await insertAdmin({
        email:
          'http-totp-invalid@example.test',
...
test(
  'enrollment challenge presented to the ordinary TOTP endpoint returns 401 invalid_challenge clears the challenge credential and creates no session',
  async () => {
    const fixtureNow =
      new Date();
    ...
    const admin =
      await insertAdmin({
        email:
          'http-totp-wrong-challenge@example.test',
...
test(
  'blocked MFA account returns 429 retains the active challenge credential and does not consume the challenge or advance the TOTP replay counter',
  async () => {
    const fixtureNow =
      new Date();
    ...
    const admin =
      await insertAdmin({
        email:
          'http-totp-throttled@example.test',
...
test(
  'replayed persisted TOTP counter maps to 401 invalid_second_factor retains the challenge credential and never creates a session',
  async () => {
    const fixtureNow =
      new Date();
